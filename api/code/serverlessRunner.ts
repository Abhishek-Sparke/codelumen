/**
 * Resilient Serverless Execution Runner for Vercel Functions
 */
...
// Built-in curated test cases fallback
const BUILTIN_TEST_CASES: Record<string, RunnerTestCase[]> = {
  'two-sum': [
    ...
    { id: 'tc-3', position: 3, isPublic: false, input: [[3, 3], 6], expectedOutput: [0, 1] }
  ],
  'p-1': [
    { id: 'tc-1', position: 1, isPublic: true, input: [[2, 7, 11, 15], 9], expectedOutput: [0, 1] },
    { id: 'tc-2', position: 2, isPublic: true, input: [[3, 2, 4], 6], expectedOutput: [1, 2] },
...
export function getProblemTestCases(problemId: string, publicOnly: boolean = false): RunnerTestCase[] {
  const cases = BUILTIN_TEST_CASES[problemId] || BUILTIN_TEST_CASES['p-1'];
  return publicOnly ? cases.filter(c => c.isPublic) : cases;
}
